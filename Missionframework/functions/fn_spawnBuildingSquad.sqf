...
params[
    ["_type", "army", [""]], ["_amount", 0, [0]], ["_positions", [],
        [
            []
        ]
    ], ["_sector", "", [""]], ["_grp", grpNull, [grpNull]], ["_classes", []]
];
...
if (count _classes > 0) then {
    _classnames = _classes;
} else {
    _classnames = [
        [] call KPLIB_fnc_getSquadComp, militia_squad
    ] select(_type == "militia");
};
