...
repair_vehicle={
	params ["_vehicle"];
    _avgdmage = (((getAllHitPointsDamage cursorObject) select 2) select {_x > 0}) call BIS_fnc_arithmeticMean;
    _damage = (ceil((_avgdmage) *100) min 20);
    _price = 0;
    _itemIndex =  (repairable_vehicles apply {tolower(_x select 0)}) find tolower(typeOf _vehicle);
    if (_itemIndex==-1) then {
        switch (true) do {
            case (_vehicle isKindOf "Car" ): {_price = 300; };
            case (_vehicle isKindOf "Tank" ): {_price = 1000; };
            case (_vehicle isKindOf "Helicopter" ): {_price = 1200; };
            case (_vehicle isKindOf "Plane" ): {_price = 2200; };
            case (_vehicle isKindOf "Boat" ): {_price = 300; };
            default {_price = 200; };
        };
    } else {
...
_player addAction [
    ...
    "
        ...
        && (typeof cursorObject) in KPLIB_b_allVeh_classes
        && count (getAllHitPointsDamage cursorObject) > 2
        && count (((getAllHitPointsDamage cursorObject) select 2) select {_x > 0}) > 0
