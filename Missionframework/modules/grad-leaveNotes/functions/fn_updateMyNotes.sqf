...
params ["_noteID", "_mode", ["_message", ""], ["_handwriting", ["",["",""]]],["_type", "note"]];
if (isnil "GRAD_leaveNotes_moduleRoot") then {
    GRAD_leaveNotes_moduleRoot = "modules\grad-leaveNotes";
};
//remove note
if (_mode == "remove") then {
    player setVariable ["GRAD_leaveNotes_notesInInventory", (player getVariable ["GRAD_leaveNotes_notesInInventory", 1]) - 1,true];
    ...
    _storeactionName = _nodeName + "_store";
    ...
    [0,["ACE_MainActions", "GRAD_leaveNotes_mainStoreAction", _storeactionName]] call FAKEACE_interact_menu_fnc_removeActionFromClass;

    _notesInventory = (player getVariable ["GRAD_leaveNotes_notesInventory", []]);
    _notesInventory deleteAt (_notesInventory find _message);
    player setVariable ["GRAD_leaveNotes_notesInventory",_notesInventory  ,true];

};

//add note
if (_mode == "add") then {
    player setVariable ["GRAD_leaveNotes_notesHandled", (player getVariable ["GRAD_leaveNotes_notesHandled", 0]) + 1,true];
    player setVariable ["GRAD_leaveNotes_notesInInventory", (player getVariable ["GRAD_leaveNotes_notesInInventory", 0]) + 1,true];
    _nodeName = format ["GRAD_leaveNotes_myNotes_%1", _noteID];
    player setVariable [_nodeName + "_message", _message,true];
    player setVariable [_nodeName + "_handwriting", _handwriting,true];
    player setVariable [_nodeName + "_type", _type,true];

    _notesInventory = (player getVariable ["GRAD_leaveNotes_notesInventory", []]);
    _notesInventory pushBack _message;
    player setVariable ["GRAD_leaveNotes_notesInventory",_notesInventory  ,true];
    ...
    _action = [_readactionName, "Read Note", GRAD_leaveNotes_moduleRoot + "\data\read.paa", _readAction, {true}, {}, [[_message, _noteID, _handwriting,_type],GRAD_leaveNotes_fnc_readNote]] call ace_interact_menu_fnc_createAction;
    ...
    _action = [_dropactionName, "Drop Note", GRAD_leaveNotes_moduleRoot + "\data\drop.paa", _dropAction, {true}, {}, [_noteID, _message, _handwriting,_type]] call ace_interact_menu_fnc_createAction;
    [player, 1, ["ACE_SelfActions", "ACE_Equipment", "GRAD_leaveNotes_mainAction", _nodeName], _action] call ace_interact_menu_fnc_addActionToObject;
    ...
    //give
    _giveactionName = _nodeName + "_give";
    _action = [_giveactionName, _actionDisplayText, GRAD_leaveNotes_moduleRoot + "\data\note.paa", {_this call GRAD_leaveNotes_fnc_giveNote}, {true}, {}, [_noteID, _message, _handwriting,_type]] call ace_interact_menu_fnc_createAction;
    ["CAManBase",0,["ACE_MainActions","GRAD_leaveNotes_mainGiveAction"],_action,true] call ace_interact_menu_fnc_addActionToClass;

     //store
    _storeactionName = _nodeName + "_store";
    _action = [_storeactionName, _actionDisplayText, GRAD_leaveNotes_moduleRoot + "\data\note.paa", {_this call GRAD_leaveNotes_fnc_storeNote}, {true}, {}, [_noteID, _message, _handwriting,_type]] call ace_interact_menu_fnc_createAction;
    ["Land_PortableCabinet_01_bookcase_black_F",0,["ACE_MainActions", "GRAD_leaveNotes_mainStoreAction"],_action,true] call ace_interact_menu_fnc_addActionToClass;
};
