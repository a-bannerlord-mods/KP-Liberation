...
_action = ["GRAD_leaveNotes_readNoteGround", "Read Note", GRAD_leaveNotes_moduleRoot + "\data\read.paa", {[[(_this select 0) getVariable ["message", ""], _this select 0, (_this select 0) getVariable ["handwriting", ["",""]], (_this select 0) getVariable ["type", "note"]],GRAD_leaveNotes_fnc_readNote] call GRAD_leaveNotes_fnc_delayedCall}, {true}] call ace_interact_menu_fnc_createAction;
