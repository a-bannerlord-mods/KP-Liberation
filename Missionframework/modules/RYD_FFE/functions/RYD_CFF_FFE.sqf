...
_phaseF = [1, 2];
if ((RydFFE_OnePhase) or((count RydFFE_FO) == 0) and not(RydFFE_2PhWithoutFO)) then {
    _phaseF = [1]
};
...
{
    if (isNil("_myFO")) exitwith {
        _waitFor = false
    };
    if (isNull _myFO) exitwith {
        _waitFor = false
    };
    ...
    if (({
            ...
            count _batlead) < 1) exitwith {
        _waitFor = false
    };
    if (isNull _battery1) exitWith {
        _waitFor = false
    };
    if (({
                (alive _x)
    ...
    if ((abs(speed _target)) > 50) exitWith {
        _waitFor = false
    };
    if ((_assumedPos select 2) > 20) exitWith {
        _waitFor = false
    };

    if ((_assumedPos distance[0, 0, 0]) == 0) exitWith {
        _waitFor = false
    };
    ...
    if (((count _phaseF) == 2) and(_x == 1) or((count _phaseF) == 1)) then {
        _spotterF = 1
    };
    ...
    [_battery, _distance, _eta, _ammoG, _batlead, _target, _markers] spawn {
        ...
        waitUntil {
            ...
            if (({
                    ...
                    count _batlead) < 1) then {
                _alive = false
            };
            if (isNull _battery1) then {
                _alive = false
            };
            if (({
                        (alive _x)
            ...
            if (_battery1 getVariable["RydFFE_Break", false]) then {
                _alive = false
            };

            {
                if not(isNull _x) then {
                    {
                        if ((vehicle _x) getVariable["RydFFE_ShotFired", false]) exitWith {
                            _shot = true
                        }
                    }
                    foreach(units _x)
        ...
        do {
            ...
            if (_battery1 getVariable["RydFFE_Break", false]) exitWith {
                _alive = false
            };
