GRAD_Fireworks = {
	...
	_nul1 say3D [_launchsound,2000];
	sleep 1;

	if (_type1 == "fizzer") then {
		_nul1 say3D [_whistlingSound,2000];
	};
	...
	_nul2 say3D [_bangSound,2000];
				...
				if (_type2 == "fizzer")  then {
					...
					for [{_j=0},{_j < (count _subfragments)},{_j=_j+1}] do 
					{
						...
						[_nul2,_type2,_subfragments,_color2,_j,_randomSleep2,_randomSleepLong2,_singleFizz1,_groupFizz1,_bangSound1] spawn {
							...
							sleep (random 1);
							_nul3 say3D [(_singleFizz2 call BIS_fnc_selectRandom),2000];
		...
		_nul2 say3D [(_groupFizz call BIS_fnc_selectRandom),2000];
		sleep 2;
		deleteVehicle _nul2;
};  
