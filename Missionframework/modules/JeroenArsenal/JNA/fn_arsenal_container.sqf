...
switch _mode do {
	...
	case "customEvents":{
		...
		_display displayAddEventHandler ["keydown",{['KeyDown',_this] call jn_fnc_arsenal_container;}];
        ...
        _ctrlButtonClose ctrladdeventhandler ["buttonclick",{["Close",[ctrlparent (_this select 0)]] call jn_fnc_arsenal_container;  _object = uiNamespace getVariable "jn_object"; ["SavetoServer",_object] call jn_fnc_arsenal;ctrlparent (_this select 0) closedisplay 2;}];
	...
	case "KeyDown": {
		...
		switch true do {
			case (_key == DIK_ESCAPE): {
				...
				// };
				["Close",[_display]] spawn jn_fnc_arsenal_container;
