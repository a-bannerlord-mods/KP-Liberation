...
switch _mode do {
    ...
    case "customEvents":{
        ...
        _ctrlButtonExport ctrlSetText "Default gear";
        _ctrlButtonExport ctrlSetTooltip "Add default loadout for your role";
        _ctrlButtonExport ctrladdeventhandler ["buttonclick",{["buttonDefaultGear",[ctrlparent (_this select 0)]] call jn_fnc_arsenal;}];
        ...
        _ctrlButtonImport ctrlSetText "Rearm Loadout";
        _ctrlButtonImport ctrlSetTooltip "Resupply your items based on last selected loadout";
        _ctrlButtonImport ctrladdeventhandler ["buttonclick",{["buttonRearmGear",[ctrlparent (_this select 0)]] call jn_fnc_arsenal;}];
        ...
        pr _inventory = if(_index == IDC_RSCDISPLAYARSENAL_TAB_CARGOMAG)then{
            ...
            _magazines = [];
            {
               ...
               // if(_itemAvailable in _usableMagazines)then{
                if(( _usableMagazines findIf { _x == toLower _itemAvailable; } != -1 ))then{
    ...
    case "UpdateItemGui":{
        ...
        if _type then{
            ...
            if(_index in [
                ...
                _compatableMagazines = ([_item] call BIS_fnc_compatibleMagazines);//TODO marker for changed entry
                {
                    pr ["_amount"];
                   ...
                   // if (_magName in _compatableMagazines) then {
                    if ( _compatableMagazines findIf { _x == toLower _magName; } != -1 ) then {                    
            ...
            case IDC_RSCDISPLAYARSENAL_TAB_HANDGUN: {
                ...
                if (_oldItem != _item) then {
                    ...
                    //_newCompatableMagazines = getarray (configfile >> "cfgweapons" >> _item >> "magazines");
                     ...
                     _oldCompatableMagazines = ([_oldItem] call BIS_fnc_compatibleMagazines);
                     _newCompatableMagazines =  ([_item] call BIS_fnc_compatibleMagazines);
                    {
                        _magazine = _x select 0;
                        _amount = _x select 1;
                        _loaded = _x select 2;
                        ...
                        if _loaded then{
                            ...
                            then{
                             ...
                             if(( _oldCompatableMagazines findIf { _x == toLower _magazine; } != -1 ))then{
                               if(( _newCompatableMagazines findIf { _x == toLower _magazine; } != -1 ))then{                                   
    ...
    case "SavetoServer": {
        ...
        pr _arsenalname = _object getVariable ["jna_name", ""];
        if (_arsenalname!="") then {
            pr _datalist = _object getVariable "jna_datalist";      
            _datalistr = str _datalist;
            Arsenal = [_arsenalname, _datalistr];
            publicVariableServer "saveArsenal";       
        };
    };
