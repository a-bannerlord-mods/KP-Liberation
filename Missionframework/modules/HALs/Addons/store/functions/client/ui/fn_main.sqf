...
switch (_mode) do {
	...
	case ("onInit"): {
		...
		private _categories = getArray (missionConfigFile >> "cfgHALsAddons" >> "cfgHALsStore" >> "stores" >> _trader getVariable ["HALs_store_trader_type", ""] >> "categories");
		{
			...
			{
				if !((_x select 0) in (_categoryItems apply {_x select 0})) then {
						if ((_x select 1) == _cat ) then {
							_categoryItems pushback [_x select 0 , (_x select 3)];
						};
					};
			} forEach GRLIB_arsenal_all;

			_categoryItems = [_categoryItems,{_x select 1},true] call HALs_fnc_sortArray;
	...
	case ("listbox"): {
		...
		switch (toLower _mode) do {
			...
			case ("sort"): {
				...
				// private _ctrlList = CTRL(IDC_LISTBOX);
				// lbSortBy  [_ctrlList,"VALUE", [true,false] select (_this select 1),false];
				// ctrlSetFocus _ctrlList;
				// _ctrlList lbSetCurSel ((_ctrlList getVariable ["idx", -1]) max 0);
			};
		};
	...
	case ("combobox"): {
		...
		switch (toLower _mode) do {
			...
			case ("update"): {
				...
				private _traderHasContainer = [KP_liberation_loadoutbox_classname] findIf {typeOf _trader isKindOf [_x, configFile >> "cfgVehicles"]} > -1;
				private _containers = [
					[
						[],
