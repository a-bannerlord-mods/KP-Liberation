...
try {
    if (isNil "HALs_store_stats_weapons") exitWith {
        ...
        HALs_store_stats_weapons = [
            ("getNumber (_x >> 'scope') isEqualTo 2 && getNumber (_x >> 'type') < 5") configClasses(configfile >> "cfgWeapons"), ["reloadtime", "dispersion", "maxzeroing", "hit", "initSpeed"],
            [true, true, false, true, false]
        ] call bis_fnc_configExtremes;

        private _statsEquipment = [
            ("getNumber (_x >> 'scope') isEqualTo 2 && getNumber (_x >> 'itemInfo' >> 'type') in [605,701,801]") configClasses(configFile >> "cfgWeapons"), ["passthrough", "armor", "maximumLoad"],
            [false, false, false]
        ] call bis_fnc_configExtremes;

        private _statsBackpacks = [
            ("getNumber (_x >> 'scope') isEqualTo 2 && getNumber (_x >> 'isBackpack') isEqualTo 1") configClasses(configFile >> "cfgVehicles"), ["passthrough", "armor", "maximumLoad"],
            [false, false, false]
        ] call bis_fnc_configExtremes;
        ...
        for "_i"
        from 2 to 3 do { //--- Ignore backpack armor and passThrough, has no effect
        ...
        private _statsOpticsMin = ("isClass (_x >> 'ItemInfo' >> 'OpticsModes')"
            configClasses(configFile >> "CfgWeapons")) apply {
            selectMax(("true"
                configClasses(_x >> "ItemInfo" >> "OpticsModes")) apply {
                getNumber(_x >> "distanceZoomMax")
            })
        };
        private _statsOpticsMax = ("isClass (_x >> 'ItemInfo' >> 'OpticsModes')"
            configClasses(configFile >> "CfgWeapons")) apply {
            selectMax(("true"
                configClasses(_x >> "ItemInfo" >> "OpticsModes")) apply {
                getNumber(_x >> "distanceZoomMax");
        ...
        _statsOpticsMin = _statsOpticsMin select {
            !isNil {
                _x
            } && _x > 0
        };
        _statsOpticsMax = _statsOpticsMax select {
            !isNil {
                _x
            } && _x > 0
        };
    ...
    call {
        if (getNumber(_config >> "type") in [1, 2, 4] && {
            ...
            private _stats = ([
                ...
                [true, true, false, true, false], _statsMin
            ] call bis_fnc_configExtremes) select 1;
            private _barMax = 1;
        ...
        if (isClass(_config >> 'ItemInfo' >> 'OpticsModes')) exitWith {
            private _zoom = selectMax(("true"
                configClasses(_config >> "ItemInfo" >> "OpticsModes")) apply {
                getNumber(_x >> "distanceZoomMax")
            });
