    ...
    if ([5] call KPLIB_fnc_hasPermission) then {
...

        {
            ...
            if ( !_next_vehicle_already_in_list ) then {
                _idact_next = _next_vehicle addAction [ "<img size='1' image='res\ui_flipveh.paa'/><t color='#FFFF00'> " + localize "STR_UNFLIP" + "</t>", "scripts\client\actions\do_unflip.sqf", "", -950, true, true, "", "build_confirmed == 0 && (_this distance _target < veh_action_distance) && (vehicle player == player)"];
