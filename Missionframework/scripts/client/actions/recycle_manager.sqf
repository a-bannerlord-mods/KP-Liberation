...
while {true} do {
    waitUntil {sleep 2; [player,1] call  KPLIB_fnc_isPlayerNearToFob};
    ...
    if ([4] call KPLIB_fnc_hasPermission) then {
        ...
        private _detected_vehicles = (getPos player) nearObjects veh_action_detect_distance select { 
            ...
            // ) && 
            ([player,1] call  KPLIB_fnc_isPlayerNearToFob) && 
            (getObjectType _x) >= 8 
        };

        {
            ...
            if (!_next_vehicle_already_in_list) then {
                private _idact_next = _next_vehicle addAction ["<img size='1' image='res\ui_recycle.paa'/><t color='#FFFF00'> " + localize "STR_RECYCLE" + "</t>", "scripts\client\actions\do_recycle.sqf", "", -900, true, true, "", "build_confirmed == 0 && ((_this distance2D _target) < veh_action_distance) && (vehicle player == player)"];
