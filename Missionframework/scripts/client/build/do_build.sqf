...
while { true } do {
    ...
    if(buildtype == 1) then {
        ...
        if ( buildtype == 8 ) then {
            ...
            _idactvector = -1;
            if (buildtype != 99 ) then {
                _idactcancel = player addAction ["<img size='1' image='res\ui_cancel.paa'/><t color='#B0FF00'> " + localize "STR_CANCEL" + "</t> ",{build_confirmed = 3; GRLIB_ui_notif = ""; hint localize "STR_CANCEL_HINT";},"",-725,false,true,"","build_confirmed == 1"];
            };
            if (buildtype == 6 ) then {
                _idactplacebis = player addAction ["<img size='1' image='res\ui_confirm.paa'/><t color='#B0FF00'> " + localize "STR_PLACEMENT_BIS" + "</t> ",{build_confirmed = 2; repeatbuild = true; hint localize "STR_CONFIRM_HINT";},"",-785,false,false,"","build_invalid == 0 && build_confirmed == 1"];
            };
            ...
            _idactrotate = player addAction ["<img size='1' image='res\ui_rotation.paa'/><t color='#B0FF00'> " + localize "STR_ROTATION" + "</t> ",{build_rotation = build_rotation + 90;},"",-750,false,false,"","build_confirmed == 1"];
            ...
            _idactplace = player addAction ["<img size='1' image='res\ui_confirm.paa'/><t color='#B0FF00'> " + localize "STR_PLACEMENT" + "</t> ",{build_confirmed = 2; hint localize "STR_CONFIRM_HINT";},"",-775,false,true,"","build_invalid == 0 && build_confirmed == 1"];
            ...
            while { build_confirmed == 1 && alive player } do {
                ...
                private _remove_objects = [];
                {
                    ...
                    if ((isObjectHidden _x) || (_x isKindOf "Animal") || (_typeOfX in GRLIB_ignore_colisions_when_building) || (_typeOfX isKindOf "CAManBase") || (isPlayer _x) || (_x == _vehicle) || ((toLower (typeOf _vehicle)) in KPLIB_b_static_classes)) then {
