...
if (GRLIB_all_fobs isEqualTo [(getposATL startbase)]) then {
    ...
    if (GRLIB_build_first_fob) then {
        ...
        while {GRLIB_all_fobs isEqualTo [(getposATL startbase)]} do {
            ...
            waitUntil {
                sleep 1;
                !(alive _fobbox) || !(GRLIB_all_fobs isEqualTo [(getposATL startbase)]) || (((getPosASL _fobbox) select 2) < 0)
            };
            sleep 10;
        };
    ...
    waitUntil {sleep 1; !(GRLIB_all_fobs isEqualTo [(getposATL startbase)])};
    ...
    for "_i" from 1 to 6 do {
        _crate = createVehicle [
            ...
            [((GRLIB_all_fobs select 1) select 0), ((GRLIB_all_fobs select 1) select 1), 150],
            [],
            80,
            "FLY"
        ];
